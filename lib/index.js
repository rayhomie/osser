...
program
  ...
  .action(({ glob, path: inpath, dir: remoteDir }) => {
    if (!glob && !inpath) {
      console.error("error: with option to upload files.");
      return;
    }
    ...
    try {
      ...
      glob && require("./mode/globMode")(glob, remoteDir, ossConfig);
      inpath && require("./mode/pathMode")(inpath, remoteDir, ossConfig);
    } catch (error) {
      console.error(
